...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            MaskDataByDomain maskDataByDomain = new MaskDataByDomain();

            ColumnMetadata columnMetadata = new ColumnMetadata();
            columnMetadata.setType("string");
            columnMetadata.setDomain("IBAN");

            maskDataByDomain = (MaskDataByDomain) maskDataByDomain.adapt(columnMetadata);

            ActionContext context = new ActionContext(null);
            maskDataByDomain.compile(context);
            Map<String, String> values = new HashMap<>();
            values.put("0001", "1");
            DataSetRow row = new DataSetRow(values);
            maskDataByDomain.applyOnColumn(row, context);
